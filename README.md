...
`npx woowa-lint`로 검증을 실시할 때 적용되는 레벨을 설정합니다.<br>
...
기존 레벨 1~3의 규칙을 수정할 수 있습니다.<br>
필요에 따라 레벨을 추가하거나 삭제할 수 있습니다.<br>
...
`woowa-mission-lint`는 Prettier를 통해 일관된 코드 스타일을 강제합니다.<br>
사용자는 `woowalint.json`의 `"프리티어_설정"` 객체를 통해 이 스타일을 **한글로** 직접 수정할 수 있습니다.

➡️ [**설정 가능한 전체 Prettier 규칙 목록은 여기를 참고하세요.**](<#🎨-프리티어-설정-(Prettier)>)

### 📝 2. 기본 린트 규칙 (ESLint)

1. `if`, `for`, `while` 같은 제어문을 사용할 때 는 중괄호를 생략할 수 없습니다.
2. 변수명/함수명은 **camelCase**를 준수해야 합니다.
3. `constants` 폴더 내의 상수명은 **영어 대문자 + SNAKE_CASE**를 준수해야 합니다.
4. console 객체를 사용할 수 없습니다.
5. 메서드 사이에는 1줄의 공백이 필요합니다.
...
`woowa-mission-lint`는 다음 라이브러리들을 기반으로 합니다.
